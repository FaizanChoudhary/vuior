import InfoIcon from "@mui/icons-material/Info";
import {
  Box,
  Container,
  Divider,
  Grid,
  IconButton,
  List,
  ListItem,
  ListItemText,
  Paper,
  Typography,
} from "@mui/material";
import React, { useEffect, useRef } from "react";
import { termsofService } from "../shared/constants"; // Ensure the path is correct

const TermsOfService = () => {
  // const [activeSection, setActiveSection] = useState(0);
  const sectionRefs = useRef([]);

  useEffect(() => {
    sectionRefs.current = sectionRefs.current.slice(
      0,
      termsofService.sections.length
    );
  }, []);

  const scrollToSection = (index) => {
    const ref = sectionRefs.current[index];
    if (ref) {
      ref.scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
    }
    // setActiveSection(index); // Set active section on click
  };

  return (
    <Container
      sx={{
        flexGrow: 1,
        padding: 4,
      }}
    >
      <Grid container spacing={3}>
        <Grid item xs={12} md={3} sx={{ display: { xs: "none", md: "block" } }}>
          <Paper
            elevation={3}
            sx={{ padding: 2, borderRadius: 2 }}
            data-aos="fade-up"
          >
            <Typography
              variant="h6"
              sx={{ fontWeight: "bold", color: "#0F4A3F", marginBottom: 2 }}
            >
              {termsofService.navigationTitle}
            </Typography>
            <List>
              {termsofService.navigationSidebar.map((item, index) => (
                <ListItem
                  button
                  key={index}
                  onClick={() => scrollToSection(index)}
                  sx={{
                    borderRadius: 2,
                    cursor: "pointer",
                    color: "#0F4A3F",
                    // backgroundColor:
                    //   activeSection === index ? "#0F4A3F" : "transparent",
                    "&:hover": { backgroundColor: "#0F4A3F", color: "white" },
                  }}
                >
                  <ListItemText primary={item} />
                </ListItem>
              ))}
            </List>
          </Paper>
        </Grid>

        <Grid item xs={12} md={9}>
          <Typography
            data-aos="fade-up"
            variant="h4"
            sx={{ fontWeight: "bold", color: "#0F4A3F", marginBottom: 2 }}
          >
            {termsofService.pageTitle}
          </Typography>

          {termsofService.sections.map((term, index) => (
            <Box
              data-aos="fade-up"
              key={index}
              ref={(el) => (sectionRefs.current[index] = el)}
              sx={{
                marginBottom: 3,
                padding: 2,
                backgroundColor: "white",
                borderRadius: 1,
                boxShadow: 2,
                transition: "0.3s",
                "&:hover": {
                  boxShadow: 3,
                },
              }}
            >
              <Box display="flex" alignItems="center">
                <IconButton sx={{ color: "#0F4A3F", padding: 0 }}>
                  <InfoIcon />
                </IconButton>
                <Typography
                  variant="h6"
                  sx={{ fontWeight: "bold", color: "#0F4A3F", marginLeft: 1 }}
                >
                  {term.title}
                </Typography>
              </Box>
              <Divider sx={{ marginY: 1 }} />
              {term.content.map((paragraph, idx) => (
                <Typography
                  key={idx}
                  variant="body1"
                  sx={{ color: "#333", marginBottom: 1 }}
                >
                  {paragraph}
                </Typography>
              ))}
            </Box>
          ))}
        </Grid>
      </Grid>
    </Container>
  );
};

export default TermsOfService;
